...
cada fecha (sin mirar al futuro), y cuenta cuantas veces se habria
disparado una señal con el umbral actual (THRESHOLD indicadores alineados).
...
from signals import evaluate_signal, THRESHOLD
...
def main():
    ...
    for i in range(MIN_HISTORY, len(all_daily_closes)):
        ...
        if signal:
            ...
            results.append((date_str, signal, count, price_then, price_after_7d))

    print(f"Dias evaluados: {len(all_daily_closes) - MIN_HISTORY}")
    print(f"Umbral usado: {THRESHOLD} indicadores")
